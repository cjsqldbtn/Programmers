...
// 문제 제목: << 전화번호 목록 (Lv.2) >>
// 트리와 비슷한 구조를 생각. 완전 탐색할 때 전체를 한 번만 훑으면 된다는 장점이 있다.
// 단점: 계속 노드를 추가하기 때문에 메모리를 많이 잡는다.
// 될.. 까? > 됐다.
...
class Solution003 {
	public boolean solution(String[] phoneBook) {
		...
		for(String i : phoneBook) {
			...
			// 노드가 생성되지 않았기에 앞부분이 동일한 문자열이 존재
			if(cnt==check) return false;
		}
		// 여기까지 왔다면 모두가 각기 다른 문자열
		return true;
	}
}
